...
class ScenarioEmulatorModel(BaseModel):
    model_config = ConfigDict(extra="forbid")
    ...
    @classmethod
    def validate_path(cls, value: list[tuple[float, tuple[float, float]]]) -> list[tuple[float, tuple[float, float]]]:
        if not value:
            raise ValueError("path must contain at least one coordinate")
        return value

    def path_index_for_time(self, t: float) -> int:
        i = 0
        while i < len(self.path) and t > self.path[i][0]:
            i += 1
...
def build_cot_event_from_scenario(
    emulator: ScenarioEmulatorModel,
    *,
    now: datetime | None = None,
    time_keeper: TimeKeeper | None = None,
    simulation_start_time: datetime | None = None,
    ...
    course: float | None = 0.0,
    geopointsrc: str = "GPS",
    altsrc: str = "GPS",
    xml_detail: str | None = None,
) -> CotEventModel:
    now = _resolve_now(now, time_keeper, simulation_start_time)
    path_index = emulator.path_index_for_time(time_keeper.get_time()) if time_keeper is not None and path_index < 0 else path_index

    if path_index < 0 or path_index >= len(emulator.path):
        raise IndexError("path_index out of range")

    _, (lat, lon) = emulator.path[path_index]

    detail = CotDetailModel(
        xml_detail=xml_detail,
        contact=ContactModel(endpoint=endpoint, callsign=emulator.callsign),
        group=emulator.group,
        precision_location=PrecisionLocationModel(
            geopointsrc=geopointsrc,
            altsrc=altsrc,
        ),
        status=StatusModel(battery=battery),
        takv=emulator.takv,
        track=TrackModel(speed=speed, course=course),
    )

    return CotEventModel(
        type_=emulator.type_,
        uid=emulator.uid,
        send_time=now,
        start_time=now,
        stale_time=now + timedelta(seconds=stale_seconds),
        how=emulator.how,
        point=CotPointModel(lat=lat, lon=lon),
        detail=detail,
        access=emulator.access,
    )


def encode_position_from_scenario(
    emulator_data: Mapping[str, Any] | Any,
    *,
    now: datetime | None = None,
    time_keeper: "TimeKeeper" | None = None,
    simulation_start_time: datetime | None = None,
    ...
    emulator = ScenarioEmulatorModel.model_validate(emulator_data)
    event = build_cot_event_from_scenario(
        emulator,
        now=now,
        time_keeper=time_keeper,
        simulation_start_time=simulation_start_time,
...
def encode_position_from_scenario_for_tcp(
    emulator_data: Mapping[str, Any] | Any,
    *,
    now: datetime | None = None,
    time_keeper: "TimeKeeper" | None = None,
