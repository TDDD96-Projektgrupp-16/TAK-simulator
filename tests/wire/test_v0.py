...
def _build_envelope() -> TakEnvelope:
    return TakEnvelope(
        event=CotEvent(
            uid="TEST-1234567890",
            type="a-f-G-U-C-I",
...
class TestV0Codec:
    ...
    def test_encode_omits_none_point_values(self):
        envelope = _build_envelope()
        ...
        assert b" hae=" not in data
        assert b" ce=" not in data
        assert b" le=" not in data
    ...
    def test_decode_missing_uid_raises_error(self):
        import pytest

        xml = b"""<?xml version='1.0'?>
<event version='2.0' type='a-u-G' time='2026-04-15T11:50:35.000Z' start='2026-04-15T11:50:40.000Z' stale='2026-04-15T11:51:50.000Z' how='h-e'>
...
<detail><contact callsign='TEST'/></detail>
    ...
    def test_decode_missing_point_raises_error(self):
        import pytest
